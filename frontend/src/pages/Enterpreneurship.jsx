import React from 'react'

const Enterpreneurship = () => {
  return (
    <div className='w-full relative min-h-screen mt-[108px]'>
      {/* Section 1 */}
      <div className="relative w-full min-h-screen flex justify-center bg-[url('/src/assets/4.png')] bg-cover bg-left md:bg-top bg-fixed md:justify-end items-center px-4 md:px-6 py-16">
        <div className="w-full sm:w-[80%] md:w-[50%] min-h-full bg-primary bg-opacity-50 px-4 md:px-8 py-10 rounded-lg flex flex-col gap-5 justify-center items-center md:items-start">
          <h1 className='w-full text-3xl md:text-4xl lg:text-5xl font-extrabold text-background [text-shadow:_0_6px_8px_#000000] text-center md:text-start'>Meet Rick Testimonials Blog Empower Your Organization with Solutions Oriented Workshops</h1>
          <h4 className='text-xl md:text-2xl font-semibold text-center md:text-start text-background'>Practical Leadership and Team Development for Lasting Results</h4>
          <p className='text-lg md:text-xl text-center md:text-start text-background font-light'>Rick Goodman’s Solutions Oriented Workshops are designed to equip your organization with the leadership, teamwork, and sales skills needed to thrive in today’s competitive business environment. With decades of experience and a proven track record, Rick’s workshops provide actionable tools to</p>
        </div>
      </div>
      {/* Section 2 */}
    <div className="w-full min-h-screen flex flex-col gap-4 justify-center items-center md:justify-start relative">
          <div className="relative w-full min-h-full px-4 md:px-12 py-14 flex flex-col md:flex-row justify-center md:justify-between items-center md:items-start bg-primary gap-6">
<div className="w-full relative md:w-[48%] min-h-full flex items-start justify-center">
<img src="/src/assets/4.png" className='w-full md:w-[80%] h-[60%] md:h-[50%] rounded-lg' alt="" />
</div>
<div className="relative w-full md:w-[48%] h-full flex flex-col gap-4 items-center md:items-start justify-center">
<h2 className='text-center md:text-start text-3xl sm:text-4xl md:text-5xl text-background font-bold tracking-wider'>
DREAMER TO DO-ER
          </h2>
          <p className='text-base md:text-lg text-background text-center md:text-start' >How far you grow is truly up to you. Whether you're interested in personal or professional growth, learning the studying personal development will rapidly accelerate growth in all aspects of your life.</p>
          <h4 className='text-lg md:text-xl text-center md:text-start text-background'>Format :</h4>
          <ul className='mb-4'>
            <li className='text-background list-disc'>30 to 60 Minutes</li>
            <li className='text-background list-disc'>Keynote or Virtual Presentation</li>
          </ul>
          <h4 className='text-lg md:text-xl text-center md:text-start text-background'>Perfect For:</h4>
          <ul className='mb-4'>
            <li className='text-background list-disc'>Entrepreneurs and Business Owners:</li>
            <li className='text-background list-disc'>Emerging leaders</li>
            <li className='text-background list-disc'>Success-driven entrepreneurs</li>
            <li className='text-background list-disc'>Corporate professionals</li>
            <li className='text-background list-disc'>Coaches and Consultants</li>
            <li className='text-background list-disc'>Experts and Professionals</li>
          </ul>
          <button className='bg-thrice text-background sm:text-xl hover:bg-tritary hover:text-primary text-base px-2 py-1 sm:px-5 text-center rounded-xl font-extrabold border-2 border-background transition-all ease-in-out duration-500'>Book Faz To Speak</button>
</div>
          </div>
    </div>

    {/* Section 3 */}
    <div className="w-full px-4 md:px-24 py-16 min-h-screen flex flex-col md:flex-row gap-6 justify-between md:justify-center items-center bg-background">
      <div className="w-full md:w-[48%] min-h-full flex flex-col gap-3 items-center md:items-start justify-center">
      <h2 className='text-center md:text-start text-3xl sm:text-4xl md:text-5xl text-primary font-bold tracking-wider'>
      GADGET'S GUIDE TO MARKETING
          </h2>
          <p className='text-base md:text-lg text-secondary text-center md:text-start' >In "GADGET's Guide to Marketing," Michael Pope leverages his extensive IT expertise to demystify technology for marketing in the coaching business. This presentation simplifies complex technological concepts and provides practical, actionable steps that yield tangible results. Michael's approach makes technology easy, understandable, and most importantly, relevant for effective marketing strategies.</p>
          <button className='bg-thrice text-background sm:text-xl hover:bg-tritary hover:text-primary text-base px-2 py-1 sm:px-5 text-center rounded-xl font-extrabold border-2 border-background transition-all ease-in-out duration-500'>Book Faz To Speak</button>
      </div>
      <div className="w-full md:w-[48%] min-h-full flex justify-center items-center">
        <img src="/src/assets/aboutpage3.jpg" alt="img" className='w-[80%] h-[300px] rounded-lg object-cover' />
      </div>
    </div>

      {/* Section 4 */}
      <div className="w-full min-h-screen flex flex-col gap-4 justify-center items-center md:justify-start relative">
          <div className="relative w-full min-h-full px-4 md:px-12 py-10 flex flex-col md:flex-row justify-center md:justify-between items-center md:items-start bg-primary gap-6">
<div className="w-full relative md:w-[48%] min-h-full flex items-start justify-center">
<img src="/src/assets/4.png" className='w-full md:w-[80%] h-[60%] md:h-[50%] rounded-lg' alt="" />
</div>
<div className="relative w-full md:w-[48%] h-full flex flex-col gap-4 items-center md:items-start justify-center">
<h2 className='text-center md:text-start text-3xl sm:text-4xl md:text-5xl text-background font-bold tracking-wider'>
DREAMER TO DO-ER
          </h2>
          <p className='text-base md:text-lg text-background text-center md:text-start' >How far you grow is truly up to you. Whether you're interested in personal or professional growth, learning the studying personal development will rapidly accelerate growth in all aspects of your life.</p>
          <h4 className='text-lg md:text-xl text-center md:text-start text-background'>Format :</h4>
          <ul className='mb-4'>
            <li className='text-background list-disc'>30 to 60 Minutes</li>
            <li className='text-background list-disc'>Keynote or Virtual Presentation</li>
          </ul>
          <h4 className='text-lg md:text-xl text-center md:text-start text-background'>Perfect For:</h4>
          <ul className='mb-4'>
            <li className='text-background list-disc'>Entrepreneurs and Business Owners:</li>
            <li className='text-background list-disc'>Emerging leaders</li>
            <li className='text-background list-disc'>Success-driven entrepreneurs</li>
            <li className='text-background list-disc'>Corporate professionals</li>
            <li className='text-background list-disc'>Coaches and Consultants</li>
            <li className='text-background list-disc'>Experts and Professionals</li>
          </ul>
          <button className='bg-thrice text-background sm:text-xl hover:bg-tritary hover:text-primary text-base px-2 py-1 sm:px-5 text-center rounded-xl font-extrabold border-2 border-background transition-all ease-in-out duration-500'>Book Faz To Speak</button>
</div>
          </div>
    </div>

     {/* Section 5 */}
     <div className="relative w-full min-h-screen flex justify-center bg-[url('/src/assets/4.png')] bg-cover bg-left md:bg-top md:justify-end items-center px-4 md:px-6 py-16">
     <div className="w-full min-h-full bg-background opacity-75 absolute top-0 left-0 z-10"></div>
        <div className="w-full sm:w-[80%] md:w-[50%] min-h-full px-4 md:px-8 py-10 rounded-lg flex flex-col gap-5 justify-center items-center md:items-start relative z-[15]">
          <h2 className='w-full text-2xl md:text-3xl lg:text-4xl font-extrabold text-primary [text-shadow:_2px_4px_4px_#fff] text-center md:text-start'>Meet Rick Testimonials Blog Empower Your Organization with Solutions Oriented Workshops</h2>
          <h4 className='text-xl md:text-2xl font-semibold text-center md:text-start text-primary'>Practical Leadership and Team Development for Lasting Results</h4>
          <p className='text-lg md:text-xl text-center md:text-start text-primary font-light'>Rick Goodman’s Solutions Oriented Workshops are designed to equip your organization with the leadership, teamwork, and sales skills needed to thrive in today’s competitive business environment. With decades of experience and a proven track record, Rick’s workshops provide actionable tools to</p>
        </div>
      </div>

      {/* Section 6 */}
      <div className="w-full min-h-screen md:min-h-[60vh] bg-[url('/src/assets/3.png')] bg-cover bg-center relative flex items-center justify-center md:justify-start">
      <div className="absolute w-full min-h-full bg-primary bg-opacity-70 z-10"></div>
      <div className="relative z-[18] flex flex-col gap-4 items-center justify-center md:items-start w-full md:w-[70%] px-4 md:px-40 py-10">
      <h2 className='w-full text-2xl md:text-3xl lg:text-4xl font-extrabold text-background [text-shadow:_2px_4px_4px_#000] text-center md:text-start'>Book Faz Now To Make Your Life Positive</h2>
      <p className='text-lg md:text-xl text-center md:text-start text-background font-light'>Rick Goodman’s Solutions Oriented Workshops are designed to equip your organization with the leadership, teamwork, and sales skills needed to thrive in today’s competitive business environment.</p>
      <button className='bg-thrice text-background sm:text-xl hover:bg-tritary hover:text-primary text-base px-2 py-1 sm:px-5 text-center rounded-xl font-extrabold border-2 border-background transition-all ease-in-out duration-500'>Book Faz To Speak</button>
      </div>
      </div>

    </div>
  )
}

export default Enterpreneurship
